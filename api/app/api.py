...
class AppAPI(MethodView):
    ...
    def post(self):
        """ Handles all posts requests for creating a new app"""

        # Checks if the request contains both an app_id and app_secret key
        ...
        # generates a password, creates the app and saves it in the DB
        else:
...
class AccessAPI(MethodView):
    """ Handles all post requests for creating a new token"""
    def __init__(self):
        if not request.json:
            abort(400)
    ...
    def post(self):
        ...
        # Checks if the request contains an app_id or app_secret key
        if not "app_id" in request.json or not "app_secret" in request.json:
            error = {
                "code": "MISSING_APP_ID_OR_APP_SECRET"
            }
        ...
        # Checks if the app actually exists using the app_id
        ...
        if not app:
            ...
            # Checks if the app_secret in the request matches the one in the DB
